function [bf, nLL] = fit_3pmodel_macro
%parameters are (1)softmax, (2)alpha, (3)bias
...
for m = 1:length(myrats)
    ...
    [initcond, bestfit, nLL{m}] = fit_bandit_model_3p(S, iters);
...
for m = 1:length(myrats)
    for j = 1:length(bf{m}(:,1))
        sc = scatter(bf{m}(j,1), bf{m}(j,2), 'MarkerFaceColor', mycolors(m,:), ...
            'MarkerEdgeColor', mycolors(m,:)); hold on
        sc.MarkerFaceAlpha = .2;
        sc.MarkerEdgeAlpha = .2;
    end
    x1 = nanmean(bf{m}(:,1))-nanstd(bf{m}(:,1));
    x2 = nanmean(bf{m}(:,1))+nanstd(bf{m}(:,1));
    y1 = nanmean(bf{m}(:,2))-nanstd(bf{m}(:,2));
    y2 = nanmean(bf{m}(:,2))+nanstd(bf{m}(:,2));
    line([x1 x2], [nanmean(bf{m}(:,2)) nanmean(bf{m}(:,2))], 'Color', mycolors(m,:));
    line([nanmean(bf{m}(:,1)) nanmean(bf{m}(:,1))], [y1 y2], 'Color', mycolors(m,:));
end
xlabel('softmax'); ylabel('alpha');
set(gca, 'TickDir', 'out'); box off;
set(gcf, 'Color', [1 1 1]);
...
for m = 1:length(myrats)
    ...
    set(gca, 'xTick', x); 
    set(gca, 'yTick', x);
    set(gca, 'xTickLabels', {'softmax'; 'alpha'; 'bias'});
    set(gca, 'yTicklabels', {'softmax'; 'alpha'; 'bias'});
    title(myrats{m});
end
set(gcf, 'Color', [1 1 1]);
