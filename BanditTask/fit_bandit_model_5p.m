function [initcond, bestfit, nLL] = fit_bandit_model_5p(S, iters)
...
for j = 1:length(data)
    ...
    for mx = 1:iters
        ...
        [initcond{j}(mx,3)] = f(rand(100,1))./100; %alpha_gain
        [initcond{j}(mx,4)] = f(rand(100,1))./5; %l/r bias
        [initcond{j}(mx,5)] = f(rand(100,1))./100; %alpha_loss
...
lower_bound = [0 0 0 -5 0];
upper_bound = [15 .3 .4 5 .4];
